...
{
    internal class RequestLoggerMiddlewareEnriched
    {
        const string MessageTemplate = "HTTP {RequestMethod} {RequestPath} responded {StatusCode} in {Elapsed:0.0000} ms";

        static readonly ILogger Log = Serilog.Log.ForContext<RequestLoggerMiddlewareEnriched>();

        readonly RequestDelegate _next;

        public RequestLoggerMiddlewareEnriched(RequestDelegate next)
        {
